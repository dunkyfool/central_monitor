...
function gpu_parser {
	...
	do
		...
		if [ "${VERSION}" != "384.111" ]; then
			GPU_NO=${LINE}
			echo "node_gpu_metrics{devid=\"${GPU_NO}\", content=\"GPU_FAN\"} 0" >> ${LOG}
			echo "node_gpu_metrics{devid=\"${GPU_NO}\", content=\"GPU_TEMP\"} 0" >> ${LOG}
			echo "node_gpu_metrics{devid=\"${GPU_NO}\", content=\"GPU_MEM\"} 0" >> ${LOG}
			echo "node_gpu_metrics{devid=\"${GPU_NO}\", content=\"GPU_MAX_MEM\"} 6075" >> ${LOG}
			echo "node_gpu_metrics{devid=\"${GPU_NO}\", content=\"GPU_PWR\"} 0" >> ${LOG}
			echo "node_gpu_metrics{devid=\"${GPU_NO}\", content=\"GPU_MAX_PWR\"} 120" >> ${LOG}
			echo "node_gpu_metrics{devid=\"${GPU_NO}\", content=\"GPU_UTIL\"} 0" >> ${LOG}
			echo "node_gpu_metrics{devid=\"${GPU_NO}\", content=\"GPU_STATUS\"} 0" >> ${LOG}
		else
			...
			echo "node_gpu_metrics{devid=\"${GPU_NO}\", content=\"GPU_STATUS\"} 1" >> ${LOG}
