...
if [ "$1" = "" ]; then
  ...
  > /home/miuser/test_pkg/monitor/txt/directory_size_bytes.prom

elif [ "$1" = "pro" ]; then
  # Logging prometheus cpu, mem
  ps -p `pgrep prometheus` -o %cpu | grep '[0-9.]'| \
  sed -ne 's/^ /node_prometheus_usage{content="cpu"} /p' \
  > /home/miuser/test_pkg/monitor/txt/prometheus_usage.prom && \
  ...
  > /home/miuser/test_pkg/monitor/txt/nginx_usage.prom && \
  ...
  > /home/miuser/test_pkg/monitor/txt/docker_disk_usage.prom
